...
class LoginOverlayWrapperElement extends OverlayElement {
  ...
  static get template() {
    if (!memoizedTemplate) {
      ...
      memoizedTemplate = super.template.cloneNode(true); // Retrieve the elements from component's template

      const thisTemplate = DomModule.import(this.is + '-template', 'template');
      const card = thisTemplate.content.querySelector('[part=card]');
      const styles = thisTemplate.content.querySelector('style'); // Append elements to cloned template
